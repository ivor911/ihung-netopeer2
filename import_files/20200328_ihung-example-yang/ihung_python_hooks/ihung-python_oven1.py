...
def process_lan_ip(it, change, old_val, new_val):
    ...
    inst_lan_ip_deleted=[]
    lan_ip_modified=[]
    inst_lan_ip_modified=[]
    lan_ip_moved=[]
    index=2
    key_index=2
    key_name="IP-Address-Index"
    if (change.oper() == sr.SR_OP_CREATED):
        ...
        while change !=  None and change.oper() == sr.SR_OP_CREATED:
                ...
                '''
                c_node = sr.Xpath_Ctx().node(c_xpath, c_node_name)
                c_node_rel = sr.Xpath_Ctx().node_rel(c_xpath, c_node_name)
                c_node_idx = sr.Xpath_Ctx().node_idx(c_xpath, index)
                c_node_idx_rel = sr.Xpath_Ctx().node_idx_rel(c_xpath, index) 
                c_next_key_name = sr.Xpath_Ctx().next_key_name(c_xpath)
                c_next_key_value = sr.Xpath_Ctx().next_key_value(c_xpath)
                c_node_key_value = sr.Xpath_Ctx().node_key_value(c_xpath, key_name)
                c_node_key_value_idx = sr.Xpath_Ctx().node_key_value_idx(c_xpath, index)
                c_key_value = sr.Xpath_Ctx().key_value(c_xpath, c_node_name, key_name)
                c_key_value_idx = sr.Xpath_Ctx().key_value_idx(c_xpath, index, key_index)

                print( "\t c_node: {}".format(c_node) )
                print( "\t c_node_rel: {}".format(c_node_rel) )
                print( "\t c_node_idx: {}".format(c_node_idx) )
                print( "\t c_node_idx_rel: {}".format(c_node_idx_rel) )
                print( "\t c_next_key_name: {}\n\n".format(c_next_key_name) )
                print( "\t c_next_key_value: {}\n\n".format(c_next_key_value) )
                print( "\t c_node_key_value: {}\n\n".format(c_node_key_value) )
                print( "\t c_node_key_value_idx: {}\n\n".format(c_node_key_value_idx) )
                print( "\t c_key_value: {}\n\n".format(c_key_value) )
                print( "\t c_key_value_idx: {}\n\n".format(c_key_value_idx) )
                '''
                lan_ip_created.append(c_val_to_string)
